import numpy as np
from ..auxiliary.errors import get_gen_error
class SaddlePoint(object):
    ...
    def _initialise(self):
        ...
        self.overlaps = {
            ...
            'bias': np.zeros(self.max_steps+1)
        }
    ...
    def _get_diff(self, t):
        ...
        diff += np.abs(self.overlaps['bias'][t+1]-self.overlaps['bias'][t])
    ...
    def iterate(self):
        ...
        for t in range(self.max_steps):
            Vtmp, qtmp, mtmp, btmp = self.model.update_se(self.overlaps['variance'][t],
            ...
            self.overlaps['bias'][t+1] = self.damp(btmp, self.overlaps['bias'][t])

            diff = self._get_diff(t)

            if any(np.isnan(val[t+1]) for val in self.overlaps.values()):
                if self.verbose:
                    print('NaN returned by saddle point update.')
                self.status = -1
                break
        ...
        self.overlaps['variance'] = self.overlaps['variance'][:t+2]
        self.overlaps['self_overlap'] = self.overlaps['self_overlap'][:t+2]
        self.overlaps['teacher_student'] = self.overlaps['teacher_student'][:t+2]
        self.overlaps['bias'] = self.overlaps['bias'][:t+1]

        self.test_error = get_gen_error(self.overlaps['self_overlap'][-1],
                                        self.overlaps['teacher_student'][-1],
                                        self.overlaps['bias'][-1],
                                        self.model.channel.prob)

        self.train_loss = self.model.channel.get_train_loss(self.overlaps['variance'][-1],
        ...
        if self.status != 0:
            info.update({
                ...
                'overlaps': {
                    ...
                    'bias': self.overlaps['bias'][-1]
                }
            })
        info.update(self.model.prior.get_info())
        info.update(self.model.channel.get_info())
